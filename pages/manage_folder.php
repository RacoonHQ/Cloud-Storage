<?php
require_once '../includes/config.php'; // Include konfigurasi utama (koneksi db, BASE_URL, dsb)
require_once '../includes/session_check.php'; // Cek apakah user sudah login
require_once '../classes/FileManager.php'; // Include class FileManager
require_once '../php/utils/functions.php'; // Include fungsi utilitas
...
// Ambil list folder untuk sidebar
        ...
        <main class="col-md-10 ms-sm-auto px-md-4">
            <div class="drive-main">
                ...
                <div class="drive-grid mt-4" id="mainGrid">
                    <!-- Folder List -->
                    <?php
                    // Tampilkan folder di dalam folder saat ini
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="list-group">
          ...
          <a href="#" id="fileActionShare" class="list-group-item list-group-item-action"><i class="bi bi-link-45deg me-2"></i>Share Link</a>
...
<script>
// Script untuk aksi modal file (preview/share/rename/delete)
...
fileActionModal.addEventListener('show.bs.modal', function (event) {
    ...
    document.getElementById('fileActionShare').href = 'share_link.php?id=' + fileId;
...
document.getElementById('searchForm').addEventListener('submit', function(e) {
    ...
    fetch('../php/file/search.php?q=' + encodeURIComponent(q))
        .then(res => res.json())
        .then(data => {
            let html = '';
            if (data.length === 0) {
                ...
                data.forEach(item => {
                    if (item.item_type === 'folder') {
                        ...
                        if (['jpg','jpeg','png','gif'].includes(ext)) {
                            let rel = 'uploads/' + <?= json_encode($_SESSION['user_id']) ?> + '/' + encodeURIComponent(item.filename);
                            icon = `<img src="../${rel}" alt="thumb" style="width:56px;height:56px;object-fit:cover;border-radius:8px;border:1.5px solid #e0e0e0;background:#fff;">`;
                        } else if (ext === 'pdf') {
                        ...
                        html += `<div class="drive-item">
                            <div>${icon}</div>
                            <div class="mt-2 fw-bold text-truncate" title="${item.filename}">${item.filename}</div>
                            ...
                            <div class="mt-2 d-flex justify-content-center flex-wrap gap-1">
                                <a href="view_file.php?id=${item.id}" class="btn btn-sm btn-outline-info" title="View"><i class="bi bi-eye"></i></a>
                                <a href="share_link.php?id=${item.id}" class="btn btn-sm btn-outline-primary" title="Share"><i class="bi bi-link-45deg"></i></a>
                                <a href="#" class="btn btn-sm btn-outline-danger" title="Delete" onclick="deleteFile(${item.id})"><i class="bi bi-trash"></i></a>
                            </div>
                        </div>`;
                    }
                });
